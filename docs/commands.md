...
**Behavior:** Creates a private channel under the configured ticket category with restricted access (requester + staff role).

---

### `/ticket_list`
View open tickets (admins only).

**Response:** Lists all open tickets with ID, user, description, status, and creation time.
...
### `/ticket_claim`
Claim a ticket (admins only).

**Parameters:**
- `ticket_id` (required): ID of the ticket to claim

**Behavior:** Assigns the ticket to the command executor and updates the ticket channel.
...
### `/ticket_close`
Close a ticket (admins only).

**Parameters:**
- `ticket_id` (required): ID of the ticket to close

**Behavior:** 
- Locks the channel for the requester
- Optionally renames channel to `ticket-<id>-closed`
- Posts GPT-generated summary embed
- Enables Delete button for admins
...
**Response:** Current version (e.g., "1.9.0 - Enhanced Reminders")
